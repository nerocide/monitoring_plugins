...
#print_hdskIOTable("Diff Table",$hdskIOTableDiff);
...
foreach my $l_key (oid_lex_sort(keys(%{$hdskIOTableDiff}))){
  ...
  if (  defined($opt_device)){
    ...
    next if ( $hdskIOTableDiff->{$dskIODevice.".".$l_val} !~ m/(${opt_device})/ );
  }

  my $IODeviceName = $hdskIOTableDiff->{$dskIODevice.".".$l_val};
  #printf("Device %s\n",$IODeviceName);
  ...
  $perfdata .=sprintf($IODeviceName."_ReadMB=%.2fMB/s ".$IODeviceName."_WriteMB=%.2fMB/s ".$IODeviceName."_ReadIO=%.2fIO/s ".$IODeviceName."_WriteIO=%.2fIO/s ".$IODeviceName."_TotalIO=%.2fIO/s ",$IONReadMBsec,$IONWrittenMBsec,$IOReadssec,$IOWritessec,$IOTotalsec);
...
# commented out in order to be able to disply IO/s for all devices
#  if ( ! defined($opt_device)){
...
#--------------------------------------------------------------------------
  ...
  foreach my $l_key (oid_lex_sort(keys(%{$hdskIOTable_2nd}))){
	...
	# if you specified a device to search do next until we found it
	if ( defined($opt_device)){
		#next if ( $l_device ne $opt_device );
		next if ( $l_device !~ m/(${opt_device})/ );
	}
...
=head1 PLUGIN HISTORY
 ...
 Version 1.4 - 2016-03-11       Check & display IO for all devices
